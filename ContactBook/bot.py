...
def bot_contact_book():
    ...
    # -----------------------------------------SEARCHING FOR CONTACTS WITH NOTES-----------------------------------------#
            ...
            if searched_contacts_with_notes:

                table = Table(title=f"Contacts with notes", expand=True, title_style=TITLE, header_style=HEADER, border_style=TABLE,
                              title_justify="center", caption_justify="center", show_lines=True, min_width=100)
        ...
        # ----------------------------------------------BIRTHDAY SEARCHING----------------------------------------------#
            ...
            if no_of_days.isdigit():
                ...
                if isinstance(list_of_contacts_with_bday, list):

                    for no, contact in enumerate(list_of_contacts_with_bday):
                        table.add_row( str(no + 1), contact["name"], contact["last_name"], contact["_date_of_birth"], contact['to_birthday'])
                    console.print(table)
                else:
                    console.print(list_of_contacts_with_bday)
